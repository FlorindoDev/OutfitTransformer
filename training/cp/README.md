...
si trova in [trainer.py](trainer.py), mentre formato e preparazione di Polyvore
sono descritti nel [README dei dati](../../data/README.md).

Torna alla [panoramica del training CP e CIR](../README.md).
...
```powershell
python -m training.cp.train_cp --max-grad-norm 1.0
```
...
```powershell
python -m training.cp.train_cp --seed 123
```
...
```powershell
python -m training.cp.train_cp --device cuda:1
```

### CPU

```powershell
python -m training.cp.train_cp --device cpu --batch-size 8 --workers 0
```
...
```powershell
python -m training.cp.train_cp --no-pretrained-image
```
...
```powershell
python -m training.cp.train_cp --batch-size 4 --workers 0
```
...
```powershell
python -m training.cp.train_cp --text-model D:\models\all-MiniLM-L6-v2
```
...
```powershell
python -m training.cp.train_cp --workers 2
python -m training.cp.train_cp --workers 4
```
